...
class _BloodPressureScreenState extends State<BloodPressureScreen> {
  ...
  void _showConnectDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text("الاتصال بجهاز قياس الضغط"),
  ...
  void _startBLEConnection() {
    ...
    targetName: _bpMonitor.deviceName,
      ...
      onData: (data) async {
        ...
        final parsed = _bpMonitor.handleData(data);
        if (data.isEmpty) {
          setState(() => _isConnecting = false);
          if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
        ...
        if (parsed != null) {
          ...
          AlertService.checkAndGenerateAlert(parsed);
  ...
  void _generatePdfReport() async {
    ...
    final file = await PdfService.generateBloodPressureReport(
      profile: profile,
      data: _allReadings,
    );
    if (!mounted) return;
  ...
  Widget _buildChart(AppLocalizations t) {
    ...
    final systolicSpots = _allReadings
        .asMap()
        .entries
        .map((e) => FlSpot(
      e.key.toDouble(),
      (e.value.value ~/ 100).toDouble(),
    ))
        .toList();

    final diastolicSpots = _allReadings
        .asMap()
        .entries
        .map((e) => FlSpot(
      e.key.toDouble(),
      ...
      body: Padding(
        ...
        child: _isConnecting
          ...
          child: Column(
            ...
            children: [
              Text("🩺 ${t.lastReading}", style: const TextStyle(fontSize: 18)),

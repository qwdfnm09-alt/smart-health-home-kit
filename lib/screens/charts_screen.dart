...
class _ChartsScreenState extends State<ChartsScreen> {
  ...
  List<HealthData> _filterData(List<HealthData> data) {
    ...
    if (selectedFilter == ChartFilter.day) {
      return data.where((d) => d.timestamp.isAfter(now.subtract(const Duration(days: 1)))).toList();
    }
    if (selectedFilter == ChartFilter.week) {
      return data.where((d) => d.timestamp.isAfter(now.subtract(const Duration(days: 7)))).toList();
    } else if (selectedFilter == ChartFilter.month) {
      return data.where((d) => d.timestamp.isAfter(now.subtract(const Duration(days: 30)))).toList();
    }
    return data;
  }
